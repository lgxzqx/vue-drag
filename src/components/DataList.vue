    ...
    <div class="attr-list">
        <el-form>
            <el-form-item v-for="(item, index) in styleKeys" :key="index" :label="item.name">
                ...
                <div class="editor" v-if="item.type == 'text'">
                ...
                <div class="list-text-box" v-if="item.type == 'list-text'">
                    <div class="list-item-box" v-for="(val,inde) in item.value" :key="inde">
                        <vue-editor :editorToolbar="customToolbar" v-model="val.title"></vue-editor>
                    ...
                    <div class="list-item-box" v-for="(val,inde) in item.value" :key="inde">
                        <el-row class="list-image-row">
                            <el-upload
                                class="avatar-uploader"
                                action="#"
                                :on-change="(file)=>onUploadChang(file, index, inde)"
                                :auto-upload="false"
                                :show-file-list="false">
    ...
    methods: {
        ...
        addFn(index, inde, type='list-text') {
            ...
            if (type == 'list-image') {
                item = {
                    title: '案例图片1',
                    src: 'https://dn160.cdn.bcebos.com/file/upload/202103/25/1616639302731246.jpg'
                }
            }
            const proValue = this.curComponent.propValue[index].value
            proValue.splice(inde, 0, item)
        },
        onUploadChang(file, index, inde = '-1') {
            const data = {
                name: file.name,
                size: file.size,
                file: file.raw
            ...
            api.uploadPic(data).then(res=> {
                ...
                if (inde != '-1') {
                    ...
                    this.curComponent.propValue[index].value = src
                }
            })
